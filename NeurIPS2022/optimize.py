...
def _write_dist_file(name, rhos, risks):
    with open(outpath+name+'.csv', 'w') as f:
        f.write("h;risk;rho_lam;rho_tnd;rho_cctnd;rho_ccpbb;rho_ccpbub;rho_ccpbskl\n")
        for i,(err,r_lam,r_tnd,r_cctnd,r_ccpbb,r_ccpbub,r_ccpbskl) in enumerate(zip(risks, rhos[0], rhos[1], rhos[2], rhos[3], rhos[4], rhos[5])):
            f.write(str(i+1)+";"+str(err)+";"+str(r_lam)+";"+str(r_tnd)+";"+str(r_cctnd)+";"+str(r_ccpbb)+";"+str(r_ccpbub)+";"+str(r_ccpbskl)+"\n")        
...
for rep in range(REPS):
    ...
    bounds, stats = rf.bounds(stats=stats, spec_bound='NO')
