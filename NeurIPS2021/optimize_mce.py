...
#
# Usage: python optimize_mce.py <data_set> [M] [sample_mode] [optimizer] [repeats]
#        M           : number of trees
...
def _write_dist_file(name, rhos, risks):
    with open(outpath+name+'.csv', 'w') as f:
        f.write("h;risk;rho_best;rho_lam;rho_tnd;rho_mu;rho_bern\n")
        for i,(err,r_best,r_lam,r_tnd,r_mu,r_bern) in enumerate(zip(risks, rhos[0], rhos[1], rhos[2], rhos[3], rhos[4])):
            f.write(str(i+1)+";"+str(err)+";"+str(r_best)+";"+str(r_lam)+";"+str(r_tnd)+";"+str(r_mu)+";"+str(r_bern)+"\n")
...
for rep in range(REPS):
    ...
    rhos.append(rho)
    print('mv_risk', mv_risk, 'KL', res_best[1]['KL'])
